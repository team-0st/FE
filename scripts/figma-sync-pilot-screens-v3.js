// Figma FE 파일럿 v3 — Body 직접 배치 + 중복 frame 정리
const K = {
...
function addText(parent, text, size = 15, bold = false, color = TX, pad = 20) {
  ...
  wrap.paddingLeft = pad;
  wrap.paddingRight = pad;
  wrap.paddingTop = 4;
...
function addCard(parent, pad = 20) {
  const wrap = figma.createFrame();
  wrap.fills = [];
  wrap.resize(375, 10);
  wrap.layoutMode = 'VERTICAL';
...
function getPage(name) { return figma.root.children.find((p) => p.name === name); }
...
for (const name of TARGETS) {
  ...
  const keep = frames.filter((f) => f.children.some((c) => c.name === 'Body')).pop() || frames[frames.length - 1];
  for (const f of frames) if (f !== keep) f.remove();
}
...
// 10 주변 상점
{
  ...
  const mt = mkText('지도 미리보기', 15, true); mapCard.appendChild(mt); mt.layoutSizingHorizontal = 'FILL';
  const mp = mkText('● ● ● ●   + 내 위치', 22, false, BR); mapCard.appendChild(mp); mp.layoutSizingHorizontal = 'FILL';
  const mh = mkText('드래그하면 가까운 4핀 갱신', 12, false, MU); mapCard.appendChild(mh); mh.layoutSizingHorizontal = 'FILL';
...
// 11 스프 결과
{
  ...
  addGap(body, 32);
  addText(body, '🍲', 56, false, TX, 0);
  addText(body, '오리지널 스프', 22, true);
  const card = addCard(body);
  const c1 = mkText('에코잼 획득', 14, true, BR); card.appendChild(c1); c1.layoutSizingHorizontal = 'FILL';
  const c2 = mkText('+30 잼', 28, true); card.appendChild(c2); c2.layoutSizingHorizontal = 'FILL';
...
// 04 홈
{
  ...
  const s1 = mkText('오늘 출석 X  ·  에코잼 10잼  ·  알맹 0P', 13, false, MU); stat.appendChild(s1); s1.layoutSizingHorizontal = 'FILL';
  const goal = addCard(body);
  const gh = figma.createFrame(); gh.layoutMode = 'HORIZONTAL'; gh.fills = []; gh.itemSpacing = 8;
  gh.appendChild(mkText('이번 주 함께 실천', 16, true));
  gh.appendChild(mkText('42%', 16, true, BL));
  goal.appendChild(gh);
  const track = figma.createFrame(); track.resize(303, 8); track.cornerRadius = 4; track.fills = [{ type: 'SOLID', color: LIGHT }]; track.clipsContent = true;
  const fill = figma.createRectangle(); fill.resize(127, 8); fill.fills = [{ type: 'SOLID', color: BR }]; track.appendChild(fill);
...
return {
  ...
  pages: TARGETS.map((n) => {
    const p = getPage(n);
    return { name: n, frames: p.children.length, hasBody: p.children[0]?.children?.some((c) => c.name === 'Body') };
  }),
};
