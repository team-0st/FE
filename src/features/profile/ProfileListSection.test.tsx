...
describe('SoupResultScreen 전체 결과 스크롤 + 고정 액션 + 재료 개수 시각화', () => {
    ...
    it('재료 2개 보상은 아이콘 2개 + 가운데 아래 라벨(재료명 n개)로 보여준다', () => {
        ...
        expect(screen.getByText('토마토 2개')).toBeTruthy();
    });

    it('에코잼 보상은 수량과 무관하게 아이콘 1개 + 에코잼 n개 라벨이다', () => {
        const ecoCraft: SoupCraftResponse = {
            ...BASE_CRAFT,
            rewardGrade: 'MEDIUM',
            rewardType: 'ECO_JAM',
            rewardAmount: 50,
        ...
        const screen = renderSoupResult(null, ecoCraft);

        expect(screen.getByLabelText('에코잼 보상')).toBeTruthy();
        expect(screen.getByText('에코잼 50개')).toBeTruthy();
        expect(screen.getByTestId('soup-result-ecojam-reward')).toBeTruthy();
    });

    it('INGREDIENT인데 rewardAmount가 에코잼처럼 커도 아이콘은 상한 내에서 수량만 쓴다', () => {
        const ingredientCraft: SoupCraftResponse = {
            ...BASE_CRAFT,
            rewardGrade: 'INGREDIENT',
            rewardIngredientId: 'tomato',
        ...
        expect(screen.getAllByLabelText(/^토마토 보상/)).toHaveLength(2);
        expect(screen.getByText('토마토 2개')).toBeTruthy();
        expect(screen.queryByText('에코잼 50개')).toBeNull();
    });
...
describe('RecipesScreen (분류 탭 고정 · 목록 영역만 스크롤)', () => {
    ...
    it('입문/일반/히든/전설/완료 탭은 이 순서로 ScrollView 밖 고정 영역에 렌더되고, ScrollView 안에는 탭이 없다', () => {
