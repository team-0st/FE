...
import { SOUP_GRADE_LABEL } from '../soup/soupRewardGrades';
...
jest.mock('@toss/tds-react-native', () => {
    ...
    const MockBottomSheetRoot = ({
        ...
        cta?: ReactNode;
        children?: ReactNode;
    }) => React.createElement(ReactNative.View, null, header, children, cta);
...
describe('CraftLanding·Gacha 화면 구조 계약 (fixed header + CenteredFeatureStage flex body + fixed footer)', () => {
    ...
    it('craft·gacha는 outer Screen ScrollView로 header+stage 전체를 감싸지 않는다 (nested ScrollView 2개만 존재)', () => {
        const craft = renderCraftLanding();
        const gacha = renderGachaIdle();

        expect(craft.UNSAFE_getAllByType(ScrollView)).toHaveLength(2);
        expect(gacha.UNSAFE_getAllByType(ScrollView)).toHaveLength(2);
    });
});
...
describe('SoupResultScreen stage/below 계약 (hero 220 viewport contain 중앙 + details 독립 스크롤)', () => {
    ...
    it('hero는 220 stage viewport 안에서 contain으로 렌더되고 고정 정사각형으로 잘리지 않는다', () => {
        const { getByTestId } = renderSoupResult();

        const bodyStyle = StyleSheet.flatten(getByTestId('soup-result-body').props.style);
        const stageStyle = StyleSheet.flatten(
            getByTestId('soup-result-stage-viewport').props.style,
        );
        expect(bodyStyle.paddingTop).toBeUndefined();
        expect(bodyStyle.paddingHorizontal).toBe(24);
        expect(stageStyle.position).toBe('absolute');
        expect(stageStyle.top).toBe('50%');
        expect(stageStyle.height).toBe(FEATURE_STAGE_HEIGHT);
        expect(stageStyle.transform).toEqual([{ translateY: -110 }]);
        ...
        const heroImage = getByTestId('soup-result-hero-image');
        expect(heroImage.props.resizeMode).toBe('contain');
        const heroStyle = StyleSheet.flatten(heroImage.props.style);
        expect(heroStyle.width).not.toBe(200);
        expect(heroStyle.height).not.toBe(200);
    });

    it('리롤 블록 유무로 below 콘텐츠 길이가 달라져도 stage viewport 위치·크기는 그대로다', () => {
        const withReroll = renderSoupResult(null);
        const stageWithReroll = StyleSheet.flatten(
            withReroll.getByTestId('soup-result-stage-viewport').props.style,
        );

        const withoutReroll = renderSoupResult({
            recipeId: 'beginner-warm',
            craft: BASE_CRAFT,
            rerollUsed: true,
        });
        const stageWithoutReroll = StyleSheet.flatten(
            withoutReroll.getByTestId('soup-result-stage-viewport').props.style,
        );

        expect(stageWithReroll).toEqual(stageWithoutReroll);
    });

    it('below 영역은 stage와 분리된 독립 ScrollView이며 기존 순서(등급→설명→리롤)를 유지한다', () => {
        const { getByTestId } = renderSoupResult();
        const below = getByTestId('soup-result-below');

        expect(below.findAllByType(ScrollView)).toHaveLength(1);
        expect(getByTestId('soup-result-stage-viewport').findAllByType(ScrollView)).toHaveLength(0);

        const texts = below.findAllByType(Text).map((node) => node.props.children);
        expect(texts).toEqual(expect.arrayContaining([SOUP_GRADE_LABEL.SMALL]));
    });
...
describe('OnboardingProfileScreen 개인정보 동의 (필수/선택 처리방침 재확인)', () => {
    ...
    function agreeOptionalPolicy(screen: OnboardingRender) {
        fireEvent.press(screen.getByLabelText(PRIVACY_POLICY_LABELS.viewFull));
        readPolicyToEnd(screen);
    ...
    it('전화번호를 입력했지만 선택 미동의 상태로 동의하고 계속을 누르면 전화번호 형식 오류보다 선택 동의 오류가 먼저 표시된다', () => {
        const screen = completePrivacyStep();

        fireEvent.changeText(screen.getByLabelText('휴대전화번호'), '123');
        fireEvent.press(screen.getByLabelText('동의하고 계속'));

        expect(screen.getByText('전화번호를 등록하려면 선택 항목에 동의해 주세요.')).toBeTruthy();
        expect(
            screen.getByLabelText(PRIVACY_POLICY_LABELS.viewFull).findAllByProps({
                color: 'red500',
                fontWeight: 'bold',
    ...
    it('선택 동의를 완료했지만 전화번호 형식이 잘못되면 기존 형식 오류가 표시된다', () => {
        const screen = completePrivacyStep();
        agreeOptionalPolicy(screen);
    ...
    it('"나중에 할게요"는 선택 동의 없이 그대로 완료할 수 있다 (회귀 없음)', () => {
        const onComplete = jest.fn();
        const screen = completePrivacyStep(onComplete);

        fireEvent.press(screen.getByLabelText('나중에 할게요'));
        fireEvent.press(screen.getByLabelText('확인했어요'));

        expect(onComplete).toHaveBeenCalledWith(
            expect.objectContaining({ almangPayoutConsent: 'declined', phoneDigits: null }),
        );
    });
});
