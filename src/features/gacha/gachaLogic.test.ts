...
describe('rollGachaReward', () => {
    it('returns shopPoints when roll is in rare band', () => {
        const sequence = [0.99, 0];
        const reward = rollGachaReward(() => sequence.shift() ?? 0);
        expect(reward.type).toBe('shopPoints');
    });

    it('returns nothing when roll is in miss band', () => {
        const reward = rollGachaReward(() => 0.1);
        expect(reward.type).toBe('nothing');
    });
});

describe('applyGachaPull', () => {
    it('deducts cost and grants eco jam', () => {
        const state = { ...DEFAULT_USER_STATE, ecoJam: 20 };
        const reward: GachaReward = { type: 'ecoJam', amount: 2 };
        const next = applyGachaPull(state, reward);
        expect(next?.ecoJam).toBe(20 - GACHA_PULL_COST_ECO_JAM + 2);
    });

    it('returns null when eco jam is insufficient', () => {
        const state = { ...DEFAULT_USER_STATE, ecoJam: 5 };
        const next = applyGachaPull(state, { type: 'nothing' });
        expect(next).toBeNull();
    });

    it('adds shop points on rare reward', () => {
        const state = { ...DEFAULT_USER_STATE, ecoJam: 15, totalPoints: 0 };
        const next = applyGachaPull(state, { type: 'shopPoints', amount: 30 });
        expect(next?.totalPoints).toBe(30);
        expect(next?.ecoJam).toBe(5);
    });
});

describe('canAffordGachaPull', () => {
    it('is false below cost', () => {
        expect(canAffordGachaPull({ ...DEFAULT_USER_STATE, ecoJam: 9 })).toBe(
            false,
        );
    });
});
