...
describe('computeStageAlphaAlignment (제작 가마솥 합성 alpha 영역을 가챠 히어로 alpha 영역과 같은 위치·높이로 맞추는 계산)', () => {
     ...
     * - 랜딩 composite(cauldron_base+soup+stir_stick) union alpha bbox: (87,23,579,540) → top=23, bottom=540 (기준: imageHeight=600)
     */
    const GACHA_HERO_ALPHA_LITERAL = { imageHeight: 395, top: 6, bottom: 390 };
    const CRAFT_COMPOSITE_ALPHA_LITERAL = { imageHeight: 600, top: 23, bottom: 540 };
    const VIEWPORT_HEIGHT_LITERAL = 220;
    const SOURCE_ASPECT_RATIO_LITERAL = 1;
    ...
    it('실측 리터럴 기준 결과 px 값이 계산된 기대값과 일치한다', () => {
        ...
        expect(alignment.innerCanvasHeight).toBeCloseTo(248.209, 3);
        expect(alignment.innerCanvasWidth).toBeCloseTo(248.209, 3);
        expect(alignment.translateY).toBeCloseTo(-6.1729, 3);
    });
