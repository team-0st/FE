...
describe('apiRequest Bearer 인증', () => {
    ...
    it('보호 API에는 Bearer 토큰만 보내고 X-Device-Id는 보내지 않는다', async () => {
        ...
        (global.fetch as jest.Mock).mockResolvedValueOnce(
            apiResponse(200, { success: true, data: { ok: true }, error: null }),
        );
    ...
    it('401이면 refresh token을 한 번 회전하고 새 access token으로 원 요청을 한 번 재시도한다', async () => {
        ...
        await expect(apiRequest<{ ok: boolean }>('/api/v1/home')).resolves.toEqual({ ok: true });
    ...
    it('등록 같은 공개 API는 인증 세션을 읽지 않는다', async () => {
        (global.fetch as jest.Mock).mockResolvedValueOnce(
            apiResponse(200, { success: true, data: { userId: 1 }, error: null }),
        );

        await apiRequest('/api/v1/users/register', { method: 'POST', withAuth: false });
